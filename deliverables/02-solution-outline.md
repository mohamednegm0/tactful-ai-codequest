...
A read-only operations dashboard that ingests patient-status events from whatever the hospital already runs (HIS like OpenMRS or local Egyptian vendors, Clinisys for lab/imaging, or — for paper-and-Excel hospitals — a tablet check-in app or nightly CSV) and renders a single live view: where every patient is right now, how long they've been there, and which stage is the bottleneck. One number per stage, one heatmap, one drill-down. No clinical decisions, no patient-facing surface in v1.
...
5. **Settings + integrations** — configurable yellow/red thresholds per stage; integration tiles showing the hospital's HIS, Vezeeta, and Clinisys (Lab) connected, WhatsApp Patient Updates marked "Coming Q2 — requires compliance partner." Frames the roadmap honestly.
...
**Buyer:** the COO / Operations Director at a mid-size private hospital (50-200 beds), or the Head of Patient Experience at a hospital group like Cleopatra-tier. **Evaluation:** 30-minute demo + 2-week pilot on one outpatient clinic; success = the director uses it daily by week 2 and books a follow-up with a second site. **Pricing hypothesis:** $400/month per facility on a self-serve tier (single clinic, CSV upload or tablet check-in, no integrations); $1,500/month on a connected tier (HIS webhooks, Clinisys integration, multi-clinic, exportable reports); custom for hospital groups. Sales cycle is short because the buyer and the user are the same person — no procurement-vs-IT split. **Onboarding scales to maturity:** Tier 1 hospitals (full HIS — e.g. Cleopatra's Clinisys deployment) wire up via webhooks in two weeks; Tier 2 (partial digital — most mid-size private hospitals) deploy a staff micro-app on tablets at each nurse station; Tier 3 (paper-and-Excel — the majority of Egyptian hospitals, only ~314 had EHR as of October 2024) start with nightly CSV upload and graduate to Tier 2 once value is proven. The expansion story is hospital groups: prove at one site, roll out to the chain. Phase 2 (WhatsApp patient updates via a Unifonic-class compliance partner — required because Saudi PDPL and similar regional laws demand healthcare-grade audit trails) becomes the upsell that justifies the connected tier.
